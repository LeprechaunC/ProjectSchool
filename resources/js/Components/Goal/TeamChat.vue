<template>
  <div class="flex flex-col h-[400px] bg-white dark:bg-gray-800 rounded-xl shadow-lg">
    <!-- Chat Header -->
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ team?.users?.length }} members</p>
          </div>
        </div>
        <button 
          v-if="$emit"
    ...
    <div class="flex-1 overflow-y-auto p-3 space-y-3" ref="messagesContainer">
      <div v-for="message in messages" :key="message.id" class="flex items-start space-x-2">
        ...
        <div class="flex-1">
          ...
          <p class="mt-0.5 text-sm text-gray-700 dark:text-gray-300">{{ message.content }}</p>
        </div>
      </div>
    </div>
    ...
    <div class="p-3 border-t border-gray-200 dark:border-gray-700">
      <form @submit.prevent="sendMessage" class="flex items-center space-x-2">
        <input
          ...
          class="flex-1 px-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white"
        />
        <button
          type="submit"
          class="p-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors duration-200"
        >
